...
def isme():
    def istmoi(ctx):
        return ctx.author.id == 131131701148647424
    return commands.check(istmoi)
...
class Debugger:
    ...
    async def interpreter(self, env, code, ctx):
        ...
        os.chdir(os.getcwd())
        with open('%s/temp.txt' % os.getcwd(), 'w') as temp:
            temp.write(body)
        ...
        try:
            ...
            if result:
                if len(str(result)) > 1950:
                    url = await hastebin(str(result).strip("`"), self.session)
    ...
    async def save(self, ctx, *, msg):
        ...
        os.chdir(os.getcwd())
        if not os.path.exists('%s/temp.txt' % os.getcwd()):
        ...
        try:
            shutil.move('%s/temp.txt' % os.getcwd(), '%s/cogs/utils/save/%s.txt' % (os.getcwd(), msg))
